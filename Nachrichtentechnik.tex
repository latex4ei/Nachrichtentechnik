...
\sectionbox{
	...
	Gleich große Quantisierungsintervalle $\mathcal I_i$ mit Breite $\Delta = g_{i+1} - g_i$\\
	Reproduktionswerte $s_i$ in der Mitte der Intervalle (midriser)\\

	\emphbox{Signalleistung $P_{\ir S} = \E[\X^2] = \int\limits_{x_{\min}}^{x_{\max}} x^2 f_{\X}(x) \diff x$}\\
	Gleichvert. $P_{\ir S} = \frac{x_{\max}^2}{3}$ \qquad Sin/Cos: $P_{\ir S} = \frac{x_{\max}^2}{2}$\\[1em]
	Fehlerleistung $P_{\ir Q} = \E[\textit{Q}^2] = \int q(x)^2 f_{\textit{Q}}(q) \diff q$\\
	$\ra$ bei gleichverteiltem Quantisierungsfehler: $P_Q = \frac{\Delta^2}{12}$

	% wichtige Formel vgl. Übung 10 $P_Q = \sum \limits_{s_i} \int \limits_{-\Delta / 2}^{\Delta / 2} z^2 f_X (z + s_i) \diff z$

	\emphbox{Signal-Noise-Ratio: $\SNR = \frac{P_S}{P_Q}$}
	
	$\SNR_{\si{\decibel}} = 10 \log_{10} \SNR = m \cdot \SI{6}{\decibel}$\\
	\\
	$\SNR_Q = \frac{P_x}{P_Q} = \begin{cases}
	...
	Optimales $\Delta = \frac{x_{\max} - x_{\min}}{2^m}$ \qquad Reprodwerte $s_i = \frac{2i - M + 1}{2} \Delta$\\
	Signal zu Quantisierungsrauschabstand: $m \cdot \SI{6}{\decibel}$
...
\sectionbox{
	\subsection{Impulsformen}
		...
		$g_{\ir NRZ}(t) = \begin{cases}
			...
			0, &\text{ sonst }			
		\end{cases}$
		\hfill $G_{\ir NRZ} = T \sinc (fT)$\\ %T \frac{\sin(\pi f T)}{\pi f T}
...
\sectionbox{	
	...
	B$_{99}$ Bandbreite: 99\% der Frequenzen (geht auch mit 90\%)\\
	...
	Beispiel GSM: $\eta = 0.88 \frac{\si{kbit/s}}{\si{\hertz}}$, \quad LTE: $\eta = \frac{3 \si{Gbit/s}}{100 \si{\mega \hertz}} = 30$\\
}
...
$r(f) = h(t) * s(t)$ \qquad $R(f) = H(f) \cdot S(f)$\\
...
\sectionbox{
	...
	Impulsantwort des Filters:\\ $h(t) = K \cdot \cxc g_s(T-t)$ \qquad (entspricht gewendetem Sendeimpuls)\\
